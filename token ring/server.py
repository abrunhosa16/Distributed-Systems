...
HOST_CALCULATOR: str  = socket.gethostbyname(socket.gethostname())
PORT_CALCULATOR: int = 12345
ADDR_CALCULATOR : tuple[str, int] = HOST_CALCULATOR, PORT_CALCULATOR
FORMAT: str = 'UTF-8'

BASE_OPERATION_1: str= '+ 2 3'
BASE_OPERATION_2: str = '* 5 5'
queue_ = queue.Queue()
...
def server_run(host: str, port: int, next_addr: tuple  , logger: logging.Logger):
    ...
    while True:
        try:
            ...
            threading.Thread(target=handle_connection, args=(client_socket, client_address, next_addr, logger)).start()

        except Exception as e:
            logger.error(f"Error accepting connection: {e}")  # Log any connection errors

# Function to handle individual client connections
def handle_connection(client: socket.socket, client_address: str, next_address: tuple[str, int], logger: logging.Logger):
    try:
        ...
        while not queue_.empty():
            ...
            calculator_server.connect(ADDR_CALCULATOR)
...
if __name__ == "__main__":
    ...
    next = sys.argv[3], int(sys.argv[4])
    log = logs(hostname)
    ...
    server_run(hostname, port, next, log.logger)
